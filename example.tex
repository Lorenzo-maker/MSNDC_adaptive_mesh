...
\end{figure}
The advantages of the $\pnh$ method, respect to the $ph$ one, are shown in Tab.~\ref{tab:tablevanderpol}, where the column "Mesh" indicates the type of discretization mesh, in particular, "Initial" refers to the initial mesh problem and $ph$ and $\pnh$ refers to the final mesh (hence, the one which guarantees the respect of the $\epsilon$ tolerance), of the~\cite{Patterson:OCAM:2015} and our method, respectively. Then "Vars" indicates the NLP variables of the resulting discretized OCP problem, "Sol. time" indicates the necessary time to solve the optimal control problem, and "Iter" refers to the number of algorithm iteration necessary to reach the given tolerance. Finally, "Tot. time" refers to the total CPU time necessary to reach the imposed convergence criterion, and $e_\text{max}$  is the maximum of the local error $\ekij$, hence
\begin{equation}
...
\begin{table}[t]
	...
	\begin{center}
		...
		\begin{tabular}{c l l l c c c}
			...
			\hline
			Mesh & Vars & Sol. time & Iter & Tot. time & $e_\text{max}$ \\
			\hline
